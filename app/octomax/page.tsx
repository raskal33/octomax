      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        ...
        >
          ...
          <div className={`mb-8 grid grid-cols-3 gap-4`}>
            {statsData.map((e, i) => {
              // Define different gradient combinations for each stat box
              const gradients = [
              ...
              // Define different shadow colors for each stat box
              const shadows = [
                "hover:shadow-[0_0_15px_rgba(20,241,149,0.3)]",
                "hover:shadow-[0_0_15px_rgba(153,69,255,0.3)]",
                "hover:shadow-[0_0_15px_rgba(0,194,255,0.3)]"
              ];
              
              // Define different text colors for each stat box
              const textColors = [
                "text-[#14F195]",
                "text-[#9945FF]",
                "text-[#00C2FF]"
              ...
              return (
                ...
                key={i}
                  ...
                  className={`col-span-4 flex h-32 w-full cursor-pointer flex-col items-center justify-center gap-4 pixel-border ${gradients[i % gradients.length]} transition-all duration-300 transform hover:scale-105 ${shadows[i % shadows.length]} xl:col-span-1`}
                  style={{ imageRendering: 'pixelated' }}
                >
                  <p className={`text-center text-sm font-bold ${textColors[i % textColors.length]} pixel-text-sm`}>
                  {e.title}
                </p>
                  <p className={`text-center text-xl font-bold text-white pixel-text-value`}>
                    {e.value}
                  </p>
                </motion.div>
              );
                      ...
                      <div
                        onClick={() =>
                          setPicks((prev) => {
                            ...
                            const filteredPicks = (prev || []).filter(p => p.id !== match.id);
                            // Add new pick if it's not the same as the one being removed
                            if (!prev?.find(p => p.id === match.id && p.pick === "odds1")) {
                              return [
                                ...filteredPicks,
                                {
                                  id: match.id,
                      ...
                      <div
                        onClick={() =>
                          setPicks((prev) => {
                            ...
                            const filteredPicks = (prev || []).filter(p => p.id !== match.id);
                            // Add new pick if it's not the same as the one being removed
                            if (!prev?.find(p => p.id === match.id && p.pick === "oddsX")) {
                              return [
                                ...filteredPicks,
                                {
                                  id: match.id,
                      ...
                      <div
                        onClick={() =>
                          setPicks((prev) => {
                            ...
                            const filteredPicks = (prev || []).filter(p => p.id !== match.id);
                            // Add new pick if it's not the same as the one being removed
                            if (!prev?.find(p => p.id === match.id && p.pick === "odds2")) {
                              return [
                                ...filteredPicks,
                                {
                                  id: match.id,
                      ...
                      <div
                        onClick={() =>
                          setPicks((prev) => {
                            ...
                            const filteredPicks = (prev || []).filter(p => p.id !== match.id);
                            // Add new pick if it's not the same as the one being removed
                            if (!prev?.find(p => p.id === match.id && p.pick === "over")) {
                              return [
                                ...filteredPicks,
                                {
                                  id: match.id,
                      ...
                      <div
                        onClick={() =>
                          setPicks((prev) => {
                            ...
                            const filteredPicks = (prev || []).filter(p => p.id !== match.id);
                            // Add new pick if it's not the same as the one being removed
                            if (!prev?.find(p => p.id === match.id && p.pick === "under")) {
                              return [
                                    ...filteredPicks,
                                    {
                                      id: match.id,
                  ...
                  >
                    ...
                    <Button
                      onClick={() => {
                        ...
                        variant="primary"
                        className="text-lg pixel-text px-6 py-3 hover:transform hover:scale-105 transition-transform duration-200 bg-gradient-to-r from-[#9945FF] to-[#00C2FF] text-black font-bold shadow-md"
                      >
                        Place Bet
                    </Button>
                  </div>
